...
{
/*
 * 定义中断/自陷向量表，并在系统初始时使程序跳转到主启动函数start
 */
	"/ low core",
	"",
  /*
   ...
   */
	...
	"",
  /*
   * 从该标识符开始的代码都位于地址空间0处，直至下一个类似的赋值标识符。
   * 那么指令br 1f就位于地址0处
   */
	". = 0^.",
  /*
   * 跳转到位于 40 地址处标号 1，再跳转到 start
   */
	"\tbr\t1f",
  /*
   * 4 为自陷指令
   * 因为内存[2，3]事实是未被使用的，而该字紧邻内存地址0和4处，
   * 可能由于指令的误差而执行到这里，那么UNIX就会执行I/O自陷指令来报告一个错误。
   **/
	"\t4",
	"",
	"/ trap vectors",
  /*
   * 设定自陷入口函数为m40.s中的trap函数，并且设定其自陷服务函数的处理器状态字为br7+0
   */ 
